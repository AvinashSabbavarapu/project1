...
class CocoDataset(Dataset):
    def __init__(self, root_dir, ann_file, vocab, transform=None):
        ...
        self.image_ids = [ann['image_id'] for ann in data['annotations']]
...
def main():
    ...
    dataset = CocoDataset(root_folder, annotation_file, vocab, transform)
    ...
    for epoch in range(num_epochs):
        ...
        for idx, (imgs, captions) in enumerate(loop):
            ...
            outputs = decoder(features, captions)  # output: B x T x V
            outputs = outputs[:, :captions.size(1) - 1, :]  # trim to target length
...
# ---------------------- Dataset Class ----------------------
class CocoDataset(Dataset):
    def __init__(self, root_dir, ann_file, vocab, transform=None, max_images=2000):
        self.root_dir = root_dir
        self.transform = transform
        self.vocab = vocab
...
def main():
    ...
    # Dataset
    vocab = Vocabulary(freq_threshold)
    full_dataset = CocoDataset(root_folder, annotation_file, vocab, transform, max_images=2000)
